...
def compute_loc_performance(
    gt_array, pred_array, eval_type, distance_tolerance, iou_threshold
):
...
def point_score(gt, pred, eval_type, distance_tolerance=20, iou_threshold=0.5):
    ...
    for scene_id in gt.keys():
        cur_tp, cur_fp, cur_fn = compute_loc_performance(
            gt[scene_id], pred[scene_id], eval_type, distance_tolerance, iou_threshold
        )
...
class SegmentF1Evaluator(object):
    def __init__(self, task, spec, detail_func=None, params=None):
        self.evaluator = BinSegmentF1Evaluator(
            task, spec, detail_func=detail_func, params=params
        )

    def evaluate(self, valid, gt, pred):
        gt = torch.nn.functional.one_hot(gt.long(), num_classes=pred.shape[1]).permute(
            0, 3, 1, 2
        )
...
class BinSegmentAccuracyEvaluator(object):
    ...
    def evaluate(self, valid, gt, pred):
        score = (
            (((pred > 0.5) == (gt > 0.5)) & valid).float().sum()
            / torch.count_nonzero(valid)
        ).item()
        self.scores.append(score.item())
...
class BinSegmentF1Evaluator(object):
    ...
    def score(self):
        ...
        for cls_idx, cls_thresholds in enumerate(self.thresholds):
            ...
            if self.detail_func:
                self.detail_func(
                    "{}_{}".format(self.task["name"], self.task["categories"][cls_idx]),
                    best_score,
                )
...
class RegressSimpleEvaluator(object):
    ...
    def evaluate(self, valid, gt, pred):
        ...
        score = (
            self.cmp_func(pred / 255, gt / 255) * valid.float()
        ).sum() / torch.count_nonzero(valid)
        self.scores.append(score.item())
            ...
            for threshold_idx, threshold in enumerate(cls_thresholds):
                ...
                for image_idx, output in enumerate(pred_raw):
                    ...
                    if len(output["boxes"]) == 0:
                        ...
                        selector = (output["scores"] >= threshold) & (
                            output["labels"] == cls_idx
                        )
...
class ClassificationF1Evaluator(object):
    def __init__(self, task, spec, detail_func=None, params=None):
        ...
        if params:
            ...
            self.thresholds = [
                list([x / 100 for x in range(1, 100)]) for _ in range(num_classes)
            ]
    ...
    def evaluate(self, gt, pred):
        for cls_idx in range(len(self.thresholds)):
            for example_idx in range(len(gt)):
                self.samples[cls_idx].append(
                    (
                        pred[example_idx, cls_idx].item(),
                        gt[example_idx].item() == cls_idx,
                    )
                )
        ...
        for cls_idx, cls_thresholds in enumerate(self.thresholds):
            ...
            for sample_prob, sample_match in cls_samples:
                if threshold_idx >= len(cls_thresholds):
                    break
                while (
                    threshold_idx < len(cls_thresholds)
                    and sample_prob > cls_thresholds[threshold_idx]
                ):
                    cur_score = get_f1()
...
def polygonize_masks(masks, boxes):
    ...
    for box_idx, (mask, box) in enumerate(zip(masks, boxes)):
        ...
        for shp, value in shapes:
            ...
            interiors = [
                [mask_to_image(x, y) for x, y in interior.coords]
...
def visualize_outputs(
    ...
    if task_type == "segment":
        ...
        if return_out:
            enc = satlas.util.encode_multiclass_binary(
                cur_outputs_bin.transpose(1, 2, 0)
            )
            raw_outputs = enc
        ...
        if evaluator_params:
            output_score_thresholds = np.array(
                [evaluator_params[category_id] for category_id in output_categories],
                dtype=np.float32,
            )
            ...
            for box_idx, polygons in enumerate(output_polygons):
                ...
                for coords in polygons:
                    ...
                    rows, cols = skimage.draw.polygon(
                        exterior[:, 1],
                        exterior[:, 0],
                        shape=(vis_output.shape[0], vis_output.shape[1]),
                    )
...
def evaluate(
    ...
    with torch.no_grad():
        ...
        for task_idx, spec in enumerate(config["Tasks"]):
            ...
            task[
                "name"
            ] = task_name  # Since we only pass the dict, not the name, to score functions.
            task_type = task["type"]
        ...
        for images, targets, info in loader:
            ...
            gpu_targets = [
                [
                    ...
                    for target_dict in cur_targets
                ]
            ...
            with torch.cuda.amp.autocast(enabled=half_enabled):
                outputs, losses = model(
                    gpu_images, gpu_targets, selected_task=selected_task
                )
            ...
            for task_idx, spec in enumerate(config["Tasks"]):
                ...
                if probs_dir:
                    for image_idx, image in enumerate(images):
                        ...
                        if task_type in ["segment", "bin_segment"]:
                            cur_outputs = (
                                outputs[task_idx][image_idx, :, :, :].cpu().numpy()
                            )
                            np.save(
                                os.path.join(
                                    probs_dir, "{}_{}.npy".format(example_id, task_name)
                                ),
                                cur_outputs,
                            )
                ...
                if task_type in ["segment", "bin_segment", "regress"]:
                    valid = torch.stack(
                        [target[task_idx]["valid"] != 0 for target in targets], dim=0
                    )
                    ...
                    valid_im = torch.stack(
                        [
                            target[task_idx]["valid_im"]
                            for i, target in enumerate(gpu_targets)
                            if valid[i]
                        ],
                        dim=0,
                    )
                    ...
                    pred = torch.stack(
                        [
                            output
                            for i, output in enumerate(outputs[task_idx])
                            if valid[i]
                        ],
                        dim=0,
                    )
                    ...
                    valid = (
                        torch.stack(
                            [target[task_idx]["valid"] for target in gpu_targets], dim=0
                        )
                        > 0
                    )
                    if torch.count_nonzero(valid) == 0:
                    ...
                    valid = (
                        torch.tensor(
                            [target[task_idx]["valid"] for target in gpu_targets]
                        )
                        > 0
                    )
                    if torch.count_nonzero(valid) == 0:
                ...
                if vis_dir or out_dir:
                    for image_idx, image in enumerate(images):
                        ...
                        if vis_output is not None:
                            skimage.io.imsave(
                                os.path.join(vis_dir, "{}_im.png".format(save_prefix)),
                                image,
                            )
                            skimage.io.imsave(
                                os.path.join(vis_dir, "{}_out.png".format(save_prefix)),
                                vis_output,
                            )
                        if vis_gt is not None:
                            skimage.io.imsave(
                                os.path.join(vis_dir, "{}_gt.png".format(save_prefix)),
                                vis_gt,
                            )
                        ...
                        if raw_outputs is not None:
                            if raw_output_format == "png":
                                skimage.io.imsave(
                                    os.path.join(out_dir, "{}.png".format(save_prefix)),
                                    raw_outputs,
                                )
                            elif raw_output_format == "npy":
                                np.save(
                                    os.path.join(out_dir, "{}.npy".format(save_prefix)),
                                    raw_outputs,
                                )
                            elif raw_output_format == "json":
                                with open(
                                    os.path.join(
                                        out_dir, "{}.json".format(save_prefix)
                                    ),
                                    "w",
                                ) as f:
                                ...
                                with open(
                                    os.path.join(out_dir, "{}.txt".format(save_prefix)),
                                    "w",
                                ) as f:
                                    f.write(raw_outputs)
