...
class AggregationBackbone(torch.nn.Module):
    def __init__(self, num_channels, backbone_cfg):
        ...
        self.backbone = backbones[underlying_cfg["Name"]](
            self.image_channels, underlying_cfg
        )
        ...
        if task_type == "segment":
            ...
            if "Weights" in head_cfg:
                class_weights = torch.tensor(
                    self.task_options[task_idx]["Weights"], dtype=torch.float32
                )
            ...
            if self.loss in ["default", "binary_cross_entropy"]:
                ...
                self.loss_func = (
                    lambda logits, targets: torch.nn.functional.cross_entropy(
                        logits, targets, reduction="none"
                    )
                )
        ...
        if task_type == "segment":
            ...
            if targets is not None:
                ...
                task_valid = torch.stack(
                    [target["valid_im"] for target in targets], dim=0
                )
                    ...
                    if not self.category_weights:
                        ...
                        if len(self.category_counts[0]) > self.count_balance:
                            ...
                            self.category_weights = [
                                np.clip(
                                    total_count / avg_count / self.num_outputs,
                                    0.01,
                                    100,
                                )
            ...
            if targets is not None:
                ...
                loss = (
                    self.loss_func(raw_outputs, task_targets.float() / 255) * task_valid
                )
                loss = loss.mean()
            ...
            if targets is not None:
                task_targets = torch.cat(
                    [target["label"] for target in targets], dim=0
                ).to(torch.long)
...
class Model(torch.nn.Module):
    def __init__(self, info):
        ...
        self.backbone = backbones[backbone_config["Name"]](
            self.num_channels, backbone_config
        )
        backbone_channels = self.backbone.out_channels

        self.intermediates = []
        for module_config in self.model_config["Intermediates"]:
            intermediate = intermediates[module_config["Name"]](
                backbone_channels, module_config
            )
        ...
        self.heads = torch.nn.ModuleList(
            self.heads
        )  # just simple for solar satlas/satlas/configs/satlas_explorer_solar_farm.txt

    def forward(self, batch_tensor, targets=None, selected_task=None):
        # Assuming batch_tensor is already a stacked tensor of images with shape [batch_size, channels, height, width]
        ...
        batch_size = batch_tensor.size(0)  # Use the batch size of the input tensor

        for task_idx, head in enumerate(self.heads):
            if selected_task and self.task_specs[task_idx]["Name"] != selected_task:
                outputs.append([None] * batch_size)
                losses.append(
                    torch.tensor(0, device=batch_tensor.device, dtype=torch.float32)
                )
                continue
            ...
            if targets:
                ...
                valid_targets = [
                    target[task_idx] for i, target in enumerate(targets) if valid[i]
                ]
                ...
                # For valid indices, use boolean indexing to select valid features and images
                valid_features = [f[valid] for f in features]
                valid_images = batch_tensor[valid]

                valid_outputs, cur_loss = head(
                    valid_images, valid_features, valid_targets
                )

                cur_outputs = [None] * batch_size
                ...
                # When there are no targets, process all images
                cur_outputs, _ = head(batch_tensor, features)
                outputs.append(cur_outputs)
