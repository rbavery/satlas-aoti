...
def handle_example(job):
    ...
    with open(os.path.join(in_dir, "gt.json")) as f:
        for p in json.load(f):
            ...
            for i in range(len(coords) - 1):
                ...
                valid = (
                    (rows >= 0) & (rows < tile_size) & (cols >= 0) & (cols < tile_size)
                )
    ...
    for category_idx in range(im.shape[2]):
        for _ in range(padding):
            im[:, :, category_idx] = skimage.morphology.binary_dilation(
                im[:, :, category_idx]
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset_root", help="Path to datasets like satlas_root/datasets/."
    )
    parser.add_argument(
    ...
    jobs = []
    for image_mode, padding in [
        ("lowres", lowres_padding),
        ("highres", highres_padding),
    ]:
