...
def is_good_image(t):
    ...
    try:
        ...
        return False
    black_fraction = (
        numpy.count_nonzero(im.max(axis=2) == 0) / im.shape[0] / im.shape[1]
    )
    white_fraction = (
        numpy.count_nonzero(im.min(axis=2) > 230) / im.shape[0] / im.shape[1]
    )
    ...
    for split in splits:
        for image_type in image_types:
            ...
            out_fname = os.path.join(
                args.satlas_root,
                "metadata",
                "good_{}_{}.json".format(image_type, split),
            )
            ...
            good = list(
                tqdm.tqdm(p.imap(is_good_image, all_chips), total=len(all_chips))
            )
            p.close()
