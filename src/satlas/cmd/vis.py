...
tasks = {task["name"]: task for task in detect_tasks + raster_tasks + property_tasks}
...
parser.add_argument(
    "--workers", help="Number of worker processes.", type=int, default=1
)
        ...
        if task_type == "point":
            ...
            for p in data:
                ...
                rows, cols = skimage.draw.polygon(
                    exterior[:, 1], exterior[:, 0], shape=(gt.shape[0], gt.shape[1])
                )
                mask[:, :] = 0
                mask[rows, cols] = 1
