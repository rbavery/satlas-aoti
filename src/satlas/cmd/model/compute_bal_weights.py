...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compute tile weights to balance examples by category frequency."
    )
    parser.add_argument(
        "--dataset_path", help="Path to datasets like /satlas/datasets/lowres/"
    )
    parser.add_argument(
        "--out_path", help="Output filename like bal_weights/lowres.json"
    )
        ...
        if task["type"] in ["regress", "classification"]:
            ...
            for example_id in os.listdir(task_dir):
                ...
                jobs.append(
                    (
                        example_name,
                        os.path.join(task_dir, example_id),
                        task["type"],
                        num_categories,
                    )
                )
            ...
            for example_name, cls_list in tqdm.tqdm(
                p.imap(get_classes, jobs), total=len(jobs)
            ):
