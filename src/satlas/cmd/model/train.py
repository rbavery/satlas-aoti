...
def main(args, config):
    ...
    train_transforms = satlas.transforms.get_transform(
        config,
        config.get(
            "TrainTransforms",
            [
    ...
    batch_transform = satlas.transforms.get_batch_transform(
        config, config.get("TrainBatchTransforms", [])
    )
    ...
    def get_task_transforms(k):
        ...
        for spec in config["Tasks"]:
            if k not in spec:
                continue
            task_transforms[spec["Name"]] = satlas.transforms.get_transform(
                config, spec[k]
            )
    ...
    val_data = satlas.model.dataset.Dataset(
        task_specs=config["Tasks"],
        transforms=satlas.transforms.get_transform(
            config, config.get("ValTransforms", [])
        ),
        channels=channels,
    ...
    if "RestorePath" in config:
        if primary:
            print("restoring from", config["RestorePath"])
    ...
    save_path = save_path.replace(
        "LABEL", os.path.basename(args.config_path).split(".")[0]
    )
    if primary:
    ...
    if "WarmupExamples" in config:
        ...
        warmup_delay_iters = (
            config.get("WarmupDelay", 0) // batch_size // args.world_size
        )
        ...
        for images, targets, info in train_loader:
            ...
            if cur_iterations % summary_iters == 0:
                ...
                if is_distributed:
                    ...
                    dist_train_loss = torch.tensor(
                        train_loss, dtype=torch.float32, device=device
                    )
                    torch.distributed.all_reduce(
                        dist_train_loss, op=torch.distributed.ReduceOp.AVG
                    )
                ...
                if primary:
                    ...
                    (
                        ...
                        val_scores,
                        _,
                    ) = satlas.model.evaluate.evaluate(
                        config=config,
                        model=model,
                        device=device,
                        loader=val_loader,
                    ...
                    print(
                        "summary_epoch {}: train_loss={} (losses={}) val_loss={} (losses={}) val={}/{} (scores={}) elapsed={},{} lr={}".format(
                            summary_epoch,
                            train_loss,
                            train_task_losses,
                            val_loss,
                    ...
                    if np.isfinite(val_score) and (
                        best_score is None or val_score > best_score
                    ):
