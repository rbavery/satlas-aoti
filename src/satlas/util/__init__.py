...
def geo_to_mercator(p, zoom=13, pixels=512):
    ...
    y = (
        (
            1
            - math.log(
                math.tan(p[1] * math.pi / 180) + (1 / math.cos(p[1] * math.pi / 180))
            )
            / math.pi
        )
        / 2
...
def interpolate_corners(tile, pixels=512, zoom=13):
    ...
    c2 = mercator_to_geo(
        (tile[0] * pixels + pixels, tile[1]), zoom=17, pixels=pixels
    )  # upper right
    c3 = mercator_to_geo(
        (tile[0], tile[1] * pixels - pixels), zoom=17, pixels=pixels
    )  # bottom left
    c4 = mercator_to_geo(
        (tile[0] * pixels + pixels, tile[1] * pixels - pixels), zoom=17, pixels=pixels
    )  # bottom right
    ...
    for i in range(start_tile[0], end_tile[0] + 1):
        for j in range(start_tile[1], end_tile[1] + 1):
            ...
            if cur_im.shape[0] < chip_size:
                ...
                cur_im = cur_im.repeat(repeats=factor, axis=0).repeat(
