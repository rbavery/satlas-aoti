...
class Vertex(object):
    ...
    def __repr__(self):
        return "Vertex({}, {}, {} in {} out)".format(
            self.id, self.point, len(self.in_edges), len(self.out_edges)
        )
...
class Edge(object):
    ...
    def is_adjacent(self, edge):
        return (
            edge.src == self.src
            or edge.src == self.dst
            or edge.dst == self.src
            or edge.dst == self.dst
        )
...
class EdgePos(object):
    ...
    def reverse(self):
        return EdgePos(
            self.edge.get_opposite_edge(), self.edge.segment().length() - self.distance
        )
...
def closest_reachable_edge(
    ...
    if src is None:
        candidates = [
            (edge, None)
            for edge in index.search(point.bounds().add_tol(distance_threshold))
        ]
    else:
    ...
    for edge, path in candidates:
        if (
            explored_node_pairs is not None
            and (
                (edge.src.id, edge.dst.id) in explored_node_pairs
                or (edge.dst.id, edge.src.id) in explored_node_pairs
            )
            and (src is None or edge != src.edge)
        ):
            continue
...
def get_graph_road_segments(g):
    ...
    def search_from_edge(rs, edge, direction):
        ...
        while True:
            ...
            edges = [
                next_edge for next_edge in edges if not next_edge.is_opposite(cur_edge)
            ]
...
def mapmatch(index, road_segments, edge_to_rs, points, segment_length):
    ...
    for edge in index.search(points[0].bounds().add_tol(START_TOL)):
        ...
        if rs.id not in probs or p > probs[rs.id][0]:
            rs_distance = rs.edge_distances[edge.id] + edge.segment().project_factor(
                points[0]
            )
    ...
    def distance_to_rs(rs, clip_distance, point, vector):
        ...
        for edge in edges:
            ...
            distance = (
                rs_point.distance(point) + edge.segment().vector().angle_to(vector) * 12
            )
    ...
    for i in xrange(len(points) - 1):
        ...
        for prev_rs_id in probs:
            ...
            rs_candidates = [(rs, prev_rs_distance)] + [
                (next_rs, -remaining_rs_distance) for next_rs in rs.out_rs(edge_to_rs)
            ]
            for next_rs, clip_distance in rs_candidates:
                if next_rs.is_opposite(rs):
                    continue
                distance, rs_distance = distance_to_rs(
                    next_rs, clip_distance, points[i + 1], points[i + 1].sub(points[i])
                )
